...
# Chargement des images et des données
@st.cache_data
def load_image(path):
    img = Image.open(path)
    buffer = BytesIO()
...
with col1:
    ...
    # Coordonnées et couleurs des points
    FIXED_WIDTH = 900
    FIXED_HEIGHT = int(FIXED_WIDTH * img_h / img_w)
    x_points, y_points, locations, hover_texts, marker_colors = [], [], [], [], []
    ...
    for lieu, pokes in pokemon_by_location.items():
        if lieu in regions_coords[region]:
            ...
            # Mettre en valeur les points correspondant à la recherche
            highlight = st.session_state.search_term.strip() == "" or any(
                st.session_state.search_term.lower() in p["name"].lower() for p in pokes
            )
            marker_colors.append("rgba(255, 215, 0, 0.8)" if highlight else "rgba(150,150,150,0.3)")
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(size=14, color=marker_colors, line=dict(width=1, color="rgba(200, 180, 0, 0.7)")),
...
with col2:
    ...
    # Affichage des Pokémon pour le lieu cliqué
    if st.session_state.clicked_location:
        lieu = st.session_state.clicked_location
        pokemons = pokemon_by_location[lieu]
        filtered_pokemons = [
            p for p in pokemons
            if st.session_state.search_term.lower() in p["name"].lower()
        ]

        st.markdown(f"<div class='pokemon-list'><h3>Pokémon à {lieu} ({len(pokemons)} au total, {len(filtered_pokemons)} filtrés)</h3></div>", unsafe_allow_html=True)
...
if st.session_state.selected_pokemon:
    ...
    if pokemon_data:
        ...
        with col2:
            ...
            type_badges = "".join(
                f'<span style="background-color:{type_colors.get(t,"#68A090")}; padding:5px 10px; border-radius:12px; color:white; margin-right:8px;">{t.capitalize()}</span>'
                for t in pokemon_data['types']
            )
            st.markdown(f"<div style='margin-bottom: 15px;'>{type_badges}</div>", unsafe_allow_html=True)

            st.markdown("#### Statistiques")
            stat_names = {'hp': 'PV', 'attack':'Attaque', 'defense':'Défense',
