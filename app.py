...
@st.cache_data
def build_db_gen1_5(_progress_callback=None):
...
def filter_local(df_local, types=None, region=None):
    ...
    if region and region != "Toutes":
        ...
        df_loc = df_loc[df_loc["generation"].astype(int).astype(str) == gen]
    if types:
        df_loc = df_loc[(df_loc["type_1"].isin(types)) | (df_loc["type_2"].isin(types))]
    return df_loc

@st.cache_data
def get_type_counts(df, types=None, region=None):
    df_loc = filter_local(df, types, region)
    type_counts = pd.Series(dtype=float)
    for col in ["type_1", "type_2"]:
        type_counts = type_counts.add(df_loc[col].value_counts(), fill_value=0)
    return type_counts.sort_values(ascending=False)

@st.cache_data
def get_stats_distribution(df, types=None, region=None):
    df_loc = filter_local(df, types, region)
    return df_loc[df_loc["total_stats"].notna()]
...
with col_update:
    if st.button("🔄 Actualiser la base (Gen1→Gen5 + items)"):
        ...
        try:
            build_db_gen1_5(_progress_callback=progress_cb)
...
df_enc = load_encounters() if os.path.exists("pokemon_location_encounters_full.csv") else None
...
with tab1:
    ...
    region = st.selectbox("Choisir une région", regions, key="region_select_tab1")
    search_term_global = st.text_input("Filtrer les lieux par Pokémon (nom)", value="", key="search_global_tab1")
    df_region = df_enc[df_enc["region"] == region] if df_enc is not None else pd.DataFrame()
...
with tab2:
    ...
    if df_pokemon is None:
        ...
        fig1.update_layout(title="Nombre de Pokémon par génération", xaxis_title="Génération", yaxis_title="Nombre")
        st.plotly_chart(fig1, use_container_width=True)

        # Séparateur visuel
        st.markdown("---")

        # Sélecteurs pour les graphiques 2 et 3 (avec clés uniques)
        ...
        type_counts = get_type_counts(df_pokemon, type_choice if type_choice else None, region_choice)
        fig2 = go.Figure()
        fig2.add_pie(labels=[t.capitalize() for t in type_counts.index], values=type_counts.values)
        fig2.update_layout(title=f"Répartition des Pokémon par type ({'Toutes régions' if region_choice=='Toutes' else region_choice})")
        ...
        df_g3 = get_stats_distribution(df_pokemon, type_choice if type_choice else None, region_choice)
        fig3 = go.Figure()
        fig3.add_histogram(x=df_g3["total_stats"], nbinsx=30)
        fig3.update_layout(
